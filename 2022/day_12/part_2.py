...
def neighbours(point: Point) -> List[Point]:
    ...
    for neighbour in neighbours:
        ...
        if value > 1 + graph[neighbour.y][neighbour.x]:
            continue
        elif graph[neighbour.y][neighbour.x] in [ord('a'), ord('S')]:
            return [neighbour]
...
def search(visited, point):
    ...
    while queue:
        ...
        for _ in range(size):
            current = queue.pop(0)
            print(f"{path_length} {graph[current.y][current.x]}=={ord('a')} {current}")
            if graph[current.y][current.x] in [ord('a'), ord('S')]:
                return path_length
